...
原因：移动端购票需要可重复登录并稳定绑定订单、常用出行人和退款权限。游客注册固定为账号、密码和手机号，登录固定为账号和密码；游客与管理员继续使用不同 session，订单归属只从后端 session 读取。

### 常用出行人与订单出行人分配

原因：移动端原流程缺少可发现的出行人入口。新增游客私有的常用出行人增删改查，并在填写信息阶段直接新增或选择；数据库增加活跃出行人时段冲突索引，避免同一证件在重叠时段重复购票。

### 游客自助退款与双操作人审计

原因：已支付游客订单需要在退票期限内自助闭环。新增游客退款接口和前端入口，复用订单退款状态机；`refund_audit_log` 增加 `operator_type` 与游客操作人外键，通过约束保证管理员和游客操作人互斥且与类型一致。为兼容 openGauss 6.0，增量字段使用条件 `DO` 块，不使用其不支持的 `ADD COLUMN IF NOT EXISTS`。

### 移动端以 iPhone 15 Pro 为主要验收规格

原因：游客端主要在手机使用。购票、订单和游客服务统一按 393 × 852 检查导航、登录/退出、底部操作栏、弹窗可滚动和横向溢出，同时保留响应式桌面布局。

### 生产发布使用版本目录和可回滚软链接

原因：直接覆盖运行目录不便于回滚。发布产物进入独立 release，切换 `current` 后重启服务；发布前保留数据库 dump，发布后必须验证 DB health、游客支付/退款、管理员设置/票种写操作和干净日志。
