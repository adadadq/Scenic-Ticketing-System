# 后台异步日报趋势 XLSX worker 设计

## 背景

日报趋势 CSV 已经接入异步导出任务，管理台同一导出入口还需要 Excel 友好的 XLSX 文件。同步日报趋势 XLSX 已经固定了聚合字段、`includeEmpty` 补零口径、表格安全边界和文件命名规则。

## 范围

- 支持单次 worker 处理一条 `DAILY_TREND + XLSX` pending 任务。
- 复用同步日报趋势 XLSX 的字段、日期校验、补零范围校验、inline string、无公式节点和 XML 1.0 非法控制字符清洗。
...
- `backend/tests/test_admin_export_jobs_api.py::test_export_job_worker_generates_daily_trend_xlsx_and_marks_job_succeeded`
