...
后台已经有汇总报表、产品维度报表和日报趋势报表。管理台需要在更长时间范围内查看月度经营走势，用于前端展示月维度趋势图和阶段复盘。本切片只提供月维度聚合数据，不进入小时粒度、任意 BI 维度或真实财务对账。
...
- 返回统一 `ApiSuccessDTO[list[AdminMonthlyTrendDTO]]`。
- 每一行按 `reportMonth` 聚合，格式为 `YYYY-MM`。
...
```text
GET /api/admin/reports/monthly-trend?dateFrom=2026-01-01&dateTo=2026-12-31
```
...
- `reportMonth`：订单创建时间所在月份，格式为 `YYYY-MM`。
- `orderCount`：该月创建的订单数。
- `paidOrderCount`：该月创建且当前支付状态为 `PAID` 的订单数。
- `completedOrderCount`：该月创建且当前订单状态为 `COMPLETED` 的订单数。
- `refundedOrderCount`：该月创建且当前订单状态为 `REFUNDED` 的订单数。
- `cancelledOrderCount`：该月创建且当前订单状态为 `CANCELLED` 的订单数。
- `netPaidAmount`：该月创建订单的当前 `paid_amount` 合计。
- `ticketCount`：该月创建订单的全部明细数。
...
3. Repository 使用参数化 SQL 聚合月维度报表。
4. Service 转换为 `AdminMonthlyTrendDTO` 列表。
5. Router 返回统一成功壳。

## 验收

- 管理员可在无 CSRF header 的情况下读取月度趋势。
...
- SQL 先按订单月份聚合金额，再按明细聚合票数，不能把订单金额因 join 放大。
- OpenAPI 记录该端点返回 `ApiSuccessDTO[list[AdminMonthlyTrendDTO]]`，并且 endpoint 清单与后端实际路由一致。
