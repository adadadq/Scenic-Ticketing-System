...
assertContains(bookingFlow.text, 'productId: selectedProduct.productId', 'order create product id')
assertContains(bookingFlow.text, 'timeSlotId: selectedSlot.id', 'order create time slot id')
...
assertContains(bookingFlow.text, '票品接口使用演示数据，暂不可下单', 'booking readiness must explain product fallback')
assertContains(bookingFlow.text, '时段接口使用演示数据，暂不可下单', 'booking readiness must explain slot fallback')
assertContains(bookingFlow.text, '临时游客需要先完成实名登记', 'booking readiness must explain real-name gate')
...
assertContains(bookingReadinessList.text, '按工作流逐步完成', 'booking readiness list must name workflow guidance')
...
assertContains(orderWorkflowStrip.text, '订单号和日期筛选只影响当前列表，不改变订单真实状态。', 'orders workflow must explain filters do not mutate order state')
...
assertContains(orderDetailSections.text, '不要在前端推断票码', 'orders detail state card must avoid frontend ticket inference')
assertContains(orderDetailSections.text, '已取消订单不会生成入园票码', 'orders detail state card must explain cancelled ticket boundary')
assertContains(orderDetailSections.text, '支付接口返回订单状态变化，请先刷新订单详情再继续操作。', 'orders detail state card must explain blocked payment refresh')
...
assertContains(adminAuthController.text, 'VITE_ADMIN_AUTH_MODE', 'admin auth mode env')
assertContains(adminAuthController.text, "import.meta.env.VITE_ADMIN_AUTH_MODE === 'api' ? 'api' : 'mock'", 'admin auth defaults to mock')
assertContains(adminAuthController.text, 'useAdminSessionQuery({ enabled: isApiMode })', 'admin auth disables API session query in mock mode')
assertContains(adminAuthController.text, 'logoutError: isApiMode ? logoutMutation.error : null', 'admin logout error exposure')
assertContains(adminOrderQueries.text, 'VITE_ADMIN_ORDERS_MODE', 'admin orders mode env')
assertContains(adminOrderQueries.text, "import.meta.env.VITE_ADMIN_ORDERS_MODE === 'api' ? 'api' : 'mock'", 'admin orders default to mock')
...
assertContains(adminRefundLogQueries.text, "import.meta.env.VITE_ADMIN_REFUND_LOGS_MODE === 'api' ? 'api' : 'mock'", 'admin refund audit search defaults to mock')
...
assertContains(adminCheckInAuditExportXlsx.text, "import.meta.env.VITE_ADMIN_CHECK_IN_LOGS_MODE === 'api' ? 'api' : 'mock'", 'admin check-in audit export defaults to mock')
...
assertContains(adminCheckInFailureLogQueries.text, "import.meta.env.VITE_ADMIN_CHECK_IN_FAILURE_LOGS_MODE === 'api' ? 'api' : 'mock'", 'admin check-in failure audit search defaults to mock')
...
assertContains(adminWorkbench.text, "admin-workbench-grid${isAuthenticated ? ' is-authenticated' : ''}", 'admin workbench authenticated responsive grid class')
assertContains(adminWorkbench.text, 'xl={isAuthenticated ? 24 : 9}', 'admin access panel becomes full width after login')
assertContains(adminWorkbench.text, 'xl={isAuthenticated ? 24 : 15}', 'admin operations panel becomes full width after login')
...
assertContains(adminWorkbench.text, '后台操作按 read-model、状态变更、审计导出分区推进', 'admin workbench operation boundary heading')
...
assertContains(adminWorkbench.text, 'admin-operation-card admin-report-workspace-card', 'admin report workspace visual class')
assertContains(adminWorkbench.text, 'className="admin-secondary-grid"', 'admin secondary operations grid class')
assertContains(adminWorkbench.text, 'admin-operation-card admin-orders-workspace-card', 'admin orders workspace visual class')
assertContains(adminWorkbench.text, 'admin-operation-card admin-audit-workspace-card', 'admin audit workspace visual class')
assertContains(adminWorkbench.text, '<AdminExportJobsPanel />', 'admin workbench composes export jobs panel')
assertContains(adminWorkbench.text, '<AdminCheckInAuditExportPanel />', 'admin workbench composes check-in audit export panel')
assertContains(adminWorkbench.text, '<AdminCheckInFailureAuditLogPanel />', 'admin workbench composes check-in failure audit search panel')
assertContains(adminExportJobsPanel.text, '异步导出任务', 'admin export jobs panel heading')
assertContains(adminExportJobsPanel.text, '创建任务必须走管理员会话和 CSRF', 'admin export jobs create CSRF boundary copy')
assertContains(adminExportJobsPanel.text, '不提交 adminUserId、任务状态、fileName、storageKey 或下载链接', 'admin export jobs internal field boundary copy')
...
assertContains(adminCss.text, '@media (max-width: 960px)', 'admin authenticated access summary must collapse before tablet widths overflow')
...
assertContains(adminCss.text, '.admin-secondary-grid', 'admin secondary grid responsive rules')
...
assertContains(adminReportCss.text, '.admin-report-filter-bar', 'admin report css must own report filter layout')
assertContains(adminReportCss.text, '.admin-payment-reconciliation-grid', 'admin report css must own payment reconciliation layout')
assertContains(adminReportCss.text, '.admin-report-table .ant-table-content', 'admin report css must keep report tables internally scrollable')
assertContains(adminReportCss.text, '@media (max-width: 768px)', 'admin report css must keep report mobile collapse rules')
assertContains(adminOrdersPanel.text, 'useAdminOrderDetailQuery(selectedOrderNo)', 'admin orders panel detail read-model query')
assertContains(adminOrdersPanel.text, 'useAdminRefundAuditLogsQuery(selectedOrderNo)', 'admin orders panel refund audit logs query')
...
assertContains(adminOrdersPanel.text, "placeholder=\"手机号后四位\"", 'admin orders panel masked phone filter')
assertContains(adminOrdersPanel.text, 'normalizeAdminPhoneFilter(buyerPhone)', 'admin orders panel normalizes masked phone filter')
assertContains(adminOrdersPanel.text, 'useAdminBatchCheckInMutation()', 'admin orders panel batch check-in mutation')
assertContains(adminOrdersPanel.text, 'useAdminBatchUndoCheckInMutation()', 'admin orders panel batch undo check-in mutation')
...
assertContains(adminOrdersPanel.text, 'useAdminPartialRefundMutation()', 'admin orders panel partial refund mutation')
assertContains(adminOrdersPanel.text, "import { AdminOrderDetailDrawer } from './components/AdminOrderDetailDrawer'", 'admin orders panel composes extracted detail drawer')
assertContains(adminOrdersPanel.text, "import { AdminBatchCheckInPanel } from './components/AdminBatchCheckInPanel'", 'admin orders panel composes batch check-in panel')
assertContains(adminOrdersPanel.text, "import { AdminBatchUndoCheckInPanel } from './components/AdminBatchUndoCheckInPanel'", 'admin orders panel composes batch undo check-in panel')
assertContains(adminOrdersPanel.text, '<AdminBatchCheckInPanel', 'admin orders panel renders batch check-in panel')
assertContains(adminOrdersPanel.text, '<AdminBatchUndoCheckInPanel', 'admin orders panel renders batch undo check-in panel')
assertContains(adminOrdersPanel.text, 'batchCheckInMutation.mutate({ ticketCodes })', 'admin orders panel submits only batch ticket codes')
assertContains(adminOrdersPanel.text, 'batchUndoCheckInMutation.mutate({', 'admin orders panel submits batch undo payload')
assertContains(adminOrdersPanel.text, '...(reason.trim() ? { reason: reason.trim() } : {})', 'admin orders panel submits optional batch undo reason')
assertContains(adminOrdersPanel.text, 'onCheckIn={(ticketCode) => checkInMutation.mutate({ ticketCode })}', 'admin order detail check-in submits ticket code only')
assertContains(adminOrdersPanel.text, '{ orderNo: selectedOrderNo, reason: refundReason }', 'admin order detail full refund submits order number and reason only')
assertContains(adminOrdersPanel.text, 'itemNos: partialRefundItemNos', 'admin order detail partial refund submits item numbers')
assertContains(adminOrdersPanel.text, 'reason: partialRefundReason', 'admin order detail partial refund submits reason')
...
assertContains(adminBatchCheckInPanel.text, '状态变更 POST，只提交 ticketCodes', 'admin batch check-in mutation boundary copy')
...
assertContains(adminBatchUndoCheckInPanel.text, '状态变更 POST，只提交 ticketCodes 和可选 reason', 'admin batch undo check-in mutation boundary copy')
...
assertContains(adminOrderRefundPanels.text, '状态变更 POST，只提交 reason', 'admin full refund mutation boundary')
assertContains(adminOrderRefundPanels.text, '退款金额、票项和库存回补由后端计算', 'admin full refund backend-computed boundary')
assertContains(adminOrderRefundPanels.text, '状态变更 POST，只提交 itemNos 和 reason', 'admin partial refund mutation boundary')
assertContains(adminOrderRefundPanels.text, '金额、状态和库存由后端计算', 'admin partial refund backend-computed boundary')
assertContains(adminRefundAuditPanel.text, '只读 GET，不提交操作人、金额或状态。', 'admin refund audit read-only boundary')
...
assertContains(adminRefundLogPanel.text, '跨订单只读 GET', 'admin refund audit search read-only boundary')
...
assertContains(adminCheckInAuditExportPanel.text, '只读 GET', 'admin check-in audit export read-only boundary')
...
assertContains(adminCheckInFailureLogPanel.text, '只读 GET', 'admin check-in failure audit search read-only boundary')
...
assertContains(adminReportQueries.text, "import.meta.env.VITE_ADMIN_REPORTS_MODE === 'api' ? 'api' : 'mock'", 'admin reports default to mock')
...
assertContains(adminReportExportCsv.text, 'adminReportExportsApi.productBreakdownCsv(normalizedParams)', 'admin reports product breakdown CSV API client')
assertContains(adminReportExportCsv.text, 'adminReportExportsApi.productBreakdownXlsx(normalizedParams)', 'admin reports product breakdown XLSX API client')
...
assertContains(adminReportsPanel.text, 'useAdminDailyTrendQuery(trendReportParams)', 'admin reports panel daily trend query uses trend filters')
assertContains(adminReportsPanel.text, 'useAdminHourlyTrendQuery(trendReportParams)', 'admin reports panel hourly trend query uses trend filters')
assertContains(adminReportsPanel.text, 'useAdminMonthlyTrendQuery(trendReportParams)', 'admin reports panel monthly trend query uses trend filters')
assertContains(adminReportsPanel.text, 'useAdminReportExports({ reportParams, trendReportParams })', 'admin reports panel delegates export state to hook')
...
assertContains(adminReportsPanel.text, 'reportExports.errors.ordersCsv', 'admin reports panel uses export hook error state')
...
assertContains(adminReportsPanel.text, 'includeEmpty: true', 'admin reports panel can enable trend zero fill')
assertContains(adminReportsPanel.text, 'canIncludeEmptyTrendBuckets', 'admin reports panel only enables zero fill with complete date range')
assertContains(adminReportsPanel.text, 'const maxTrendZeroFillDays = 31', 'admin reports panel respects hourly trend zero fill range limit')
assertContains(adminReportsPanel.text, 'countInclusiveDays(reportParams.dateFrom, reportParams.dateTo)', 'admin reports panel counts zero fill date range')
assertContains(adminReportsPanel.text, 'disabled={!canIncludeEmptyTrendBuckets}', 'admin reports panel disables zero fill when range is invalid')
assertContains(adminReportsPanel.text, 'aria-label="报表开始日期筛选"', 'admin reports panel dateFrom accessible input')
assertContains(adminReportsPanel.text, 'aria-label="报表结束日期筛选"', 'admin reports panel dateTo accessible input')
assertContains(adminReportsPanel.text, 'className="admin-report-include-empty"', 'admin reports panel zero fill switch class')
assertContains(adminReportsPanel.text, '重置报表筛选', 'admin reports panel reset filters action')
assertContains(adminReportSummaryMetrics.text, 'reportParams', 'admin reports summary metrics uses current filter fallback')
assertContains(adminReportsPanel.text, "import { AdminReportTrendPanel } from './components/AdminReportTrendPanel'", 'admin reports panel composes trend panel')
assertContains(adminReportsPanel.text, "import { AdminReportProductPanel } from './components/AdminReportProductPanel'", 'admin reports panel composes product panel')
assertContains(adminReportsPanel.text, 'hourlyTrendQuery.refetch()', 'admin reports panel refetches hourly trend')
assertContains(adminReportsPanel.text, 'reconciliationQuery.refetch()', 'admin reports panel refetches payment reconciliation')
assertContains(adminReportsPanel.text, 'title="小时趋势"', 'admin reports panel renders hourly trend title')
assertContains(adminReportsPanel.text, 'subtitle="按订单创建小时聚合"', 'admin reports panel renders hourly trend subtitle')
assertContains(adminReportsPanel.text, 'monthlyTrendQuery.refetch()', 'admin reports panel refetches monthly trend')
assertContains(adminReportsPanel.text, 'title="月度趋势"', 'admin reports panel renders monthly trend title')
assertContains(adminReportsPanel.text, 'subtitle="按订单创建月聚合"', 'admin reports panel renders monthly trend subtitle')
...
assertContains(adminReportsPanel.text, 'className="admin-report-export-action"', 'admin reports panel CSV export button class')
assertContains(adminReportsPanel.text, 'className="admin-report-xlsx-export-action"', 'admin reports panel XLSX export button class')
assertContains(adminReportsPanel.text, 'AdminPaymentReconciliationPanel', 'admin reports panel composes payment reconciliation panel')
assertContains(adminReportsPanel.text, 'AdminReportTrendExportBar', 'admin reports panel composes trend export bar')
...
assertContains(adminReportsPanel.text, '导出订单 CSV', 'admin reports panel CSV export button copy')
assertContains(adminReportsPanel.text, '导出订单 XLSX', 'admin reports panel XLSX export button copy')
assertContains(adminReportsPanel.text, '支付对账 CSV 导出失败', 'admin reports panel payment reconciliation CSV error copy')
assertContains(adminReportsPanel.text, '支付对账 CSV 暂时无法导出', 'admin reports panel payment reconciliation CSV fixed error details')
assertContains(adminReportsPanel.text, '产品维度 CSV 导出失败', 'admin reports panel product breakdown CSV error copy')
assertContains(adminReportsPanel.text, '产品维度 XLSX 导出失败', 'admin reports panel product breakdown XLSX error copy')
assertContains(adminReportsPanel.text, '产品维度 XLSX 暂时无法导出', 'admin reports panel product breakdown XLSX fixed error details')
assertContains(adminReportsPanel.text, '产品维度 CSV 暂时无法导出', 'admin reports panel product breakdown CSV fixed error details')
assertContains(adminReportsPanel.text, '支付对账 XLSX 导出失败', 'admin reports panel payment reconciliation XLSX error copy')
assertContains(adminReportsPanel.text, '支付对账 XLSX 暂时无法导出', 'admin reports panel payment reconciliation XLSX fixed error details')
assertContains(adminReportsPanel.text, '趋势 XLSX 导出失败', 'admin reports panel trend XLSX error copy')
assertContains(adminReportsPanel.text, '趋势 XLSX 暂时无法导出', 'admin reports panel trend XLSX fixed error details')
assertContains(adminReportTrendExportBar.text, '趋势 CSV/XLSX 导出跟随当前日期范围和补零口径', 'admin reports trend export bar scope copy')
assertContains(adminReportsPanel.text, '报表接口为只读 GET', 'admin reports panel read-only boundary')
...
const {
  ...
  escapeCsvCell,
  neutralizeSpreadsheetFormulaText,
} = loadRuntimeFunctions([adminReportXlsxWorkbook.sourceFile, adminReportExportCsv.sourceFile], [
