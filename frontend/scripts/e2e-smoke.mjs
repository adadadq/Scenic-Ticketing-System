...
import { createE2eOrderFactory } from './e2e-mock-api.mjs'
...
const createOrder = createE2eOrderFactory({ phone: e2ePhone, visitorName: e2eVisitorName })
...
async function run() {
  await withE2eHarness({
    ...
    let databaseHealthFailureState = null
    let sessionFailureState = null
    if (mock) {
      mock.state.databaseHealthFail = true
      await navigateApp(client, appUrl)
      await waitFor(client, 'document.readyState === "complete"', 'page load with database health failure')
      await waitFor(client, headerIncludesText('数据库异常'), 'database health failure badge')
      await waitFor(
        client,
        `(() => {
          const badge = document.querySelector('.status-diagnostic-badge.is-error')
          badge?.focus()
          return document.activeElement === badge
        })()`,
        'database diagnostic badge focus',
      )
      await waitFor(client, includesText('数据库健康检查失败'), 'database diagnostic title')
      await waitFor(client, includesText('错误码：DATABASE_UNAVAILABLE'), 'database diagnostic code')
      await waitFor(client, includesText('请求编号：e2e-request'), 'database diagnostic request id')
      await waitFor(client, includesText('手机号登录'), 'login entry during database health failure')
      await waitFor(client, `Boolean(document.querySelector('.ticket-card-title'))`, 'ticket card during database health failure')
      databaseHealthFailureState = await evaluate(
        client,
        `(() => ({
          hasDiagnosticCode: document.body.textContent.includes('错误码：DATABASE_UNAVAILABLE'),
          hasDiagnosticRequestId: document.body.textContent.includes('请求编号：e2e-request'),
          hasDatabaseError: document.querySelector('.app-header')?.textContent.includes('数据库异常') ?? false,
          hasLoginEntry: document.body.textContent.includes('手机号登录'),
          hasTicketCard: Boolean(document.querySelector('.ticket-card-title')),
        }))()`,
      )
      mock.state.databaseHealthFail = false
      await navigateApp(client, appUrl)
      await waitFor(client, 'document.readyState === "complete"', 'page load after database health recovery')

      mock.state.sessionLookupFail = true
      await navigateApp(client, appUrl)
      await waitFor(client, 'document.readyState === "complete"', 'page load with session lookup failure')
      await waitFor(client, headerIncludesText('会话异常'), 'session failure status badge')
      await waitFor(client, headerIncludesText('会话检查失败'), 'auth session failure entry')
      await openAuthSessionDiagnostic(client, 'logged-out auth session')
      await waitFor(client, includesText('游客会话检查失败'), 'auth session diagnostic title')
      ...
      sessionFailureState = await evaluate(
        ...
        `(() => ({
          ...
          hasSessionError: document.querySelector('.auth-status')?.textContent.includes('会话检查失败') ?? false,
        }))()`,
      )
      mock.state.sessionLookupFail = false
      await clickSelector(client, '.auth-status button:not(.auth-session-error-trigger)')
      await waitFor(client, headerIncludesText('游客未登录'), 'session recovery after retry')
    }

    await waitFor(client, headerIncludesText(mock ? 'API 正常 · e2e' : 'API 正常'), 'health status badge')
    await waitFor(client, headerIncludesText('DB 正常'), 'database health status badge')
    await waitFor(client, headerIncludesText('游客未登录'), 'logged-out session badge')
    await waitFor(client, includesText('手机号登录'), 'login entry')
    ...
    if (mock) {
      ...
      await waitFor(client, `document.querySelector('.page-heading h1')?.textContent.includes('游客购票工作台')`, 'booking page from logged-out orders')
    }
    ...
    await waitFor(client, hasPlaceholder('13911112222'), 'login phone input', 10000)
    await fillPlaceholder(client, '13911112222', '139 1111a2222')
    await clickByText(client, '登录为临时游客')
    await waitFor(client, includesText('请输入 11 位中国大陆手机号'), 'invalid login phone validation')
    if (mock) {
      assert(mock.state.loginBodies.length === 0, 'invalid login phone should not submit request in mock mode')
    }

    await fillPlaceholder(client, '13911112222', e2ePhoneWithSeparators)
    if (mock) {
      mock.state.rateLimitedLoginPhones.add(e2ePhone)
    }
    await clickByText(client, '登录为临时游客')
    if (mock) {
      ...
      await waitFor(
        ...
        `(() => {
          ...
          return authText.includes('未登录') && !authText.includes('临时游客')
        })()`,
      ...
      mock.state.rateLimitedLoginPhones.delete(e2ePhone)
      await clickByText(client, '登录为临时游客')
    }
    await waitFor(client, headerIncludesText('临时游客会话'), 'temporary visitor session badge')
    if (mock) {
      mock.state.sessionLookupFail = true
      await navigateApp(client, appUrl)
      await waitFor(client, 'document.readyState === "complete"', 'temporary visitor page load with session lookup failure')
      await waitFor(client, headerIncludesText('会话异常'), 'temporary visitor session refetch failure badge')
      await waitFor(client, headerIncludesText('会话检查失败'), 'temporary visitor auth session failure entry')
      await openAuthSessionDiagnostic(client, 'temporary visitor auth session')
      await waitFor(client, includesText('错误码：SESSION_LOOKUP_FAILED'), 'temporary visitor auth session diagnostic code')
      await waitFor(client, includesText('请求编号：e2e-request'), 'temporary visitor auth session diagnostic request id')
      mock.state.sessionLookupFail = false
      await clickSelector(client, '.auth-status button:not(.auth-session-error-trigger)')
      await waitFor(client, headerIncludesText('临时游客会话'), 'temporary visitor session recovery after retry')
      ...
      await waitFor(
        ...
        `(() => {
          ${visibleElementScript}
          const button = document.querySelector('.pay-button')
          return Boolean(button && isVisible(button) && button.textContent.includes('去实名登记') && !button.disabled)
        })()`,
        'desktop temporary visitor real-name booking action',
      )
      desktopAuthActionState.registerLabel = await evaluate(client, `document.querySelector('.pay-button')?.textContent || ''`)
      await clickSelector(client, '.pay-button')
    } else {
      await waitFor(
        client,
        `(() => {
          const button = document.querySelector('.mobile-action-bar button')
          return Boolean(button?.textContent.includes('去实名') && !button.disabled)
        })()`,
        'temporary visitor real-name booking action',
      )
      await clickSelector(client, '.mobile-action-bar button')
    }
    await waitFor(client, hasPlaceholder('张三'), 'register name input')
    await waitFor(client, enabledSelector('.realname-confirm-checkbox'), 'real-name confirmation checkbox')
    await waitFor(
      client,
      `Boolean(document.querySelector('.realname-submit-button')?.disabled)`,
      'real-name submit disabled before confirmation',
    )
    await fillPlaceholder(client, '张三', e2eVisitorName)
    await fillPlaceholder(client, '11010519491231002X', '110105194912310021')
    await fillPlaceholder(client, '13911112222', e2ePhoneWithSeparators)
    await waitFor(
      client,
      `Boolean(document.querySelector('.realname-submit-button')?.disabled)`,
      'real-name submit still disabled after required fields',
    )
    if (mock) {
      assert(mock.state.registerBodies.length === 0, 'unchecked real-name confirmation should block register request in mock mode')
    }

    await clickSelector(client, '.realname-confirm-checkbox')
    await waitFor(client, enabledSelector('.realname-submit-button'), 'real-name submit enabled after confirmation')
    await clickSelector(client, '.realname-submit-button')
    await waitFor(client, includesText('身份证日期或校验位不正确'), 'invalid id card validation')
    if (mock) {
      assert(mock.state.registerBodies.length === 0, 'invalid id card should not submit register request in mock mode')
    }

    await fillPlaceholder(client, '11010519491231002X', e2eIdNumber)
    await fillPlaceholder(client, '13911112222', '139 1111a2222')
    await clickSelector(client, '.realname-submit-button')
    ...
    await clickSelector(client, '.realname-submit-button')
    if (mock) {
      await waitFor(client, includesText('实名信息不可用'), 'register conflict error')
      await waitFor(client, includesText('请求编号：e2e-request'), 'register conflict request id')
      await waitFor(client, headerIncludesText('临时游客会话'), 'register conflict keeps temporary session badge')
      await waitFor(
        client,
        `document.querySelector('.auth-status')?.textContent.includes('Temporary Visitor')`,
        'register conflict keeps temporary visitor identity',
      )
      assert(mock.state.visitor?.visitorScope === 'TEMP', 'register conflict should not upgrade visitor scope in mock mode')
      mock.state.registerConflictPhones.delete(e2ePhone)
      await clickSelector(client, '.realname-submit-button')
    }
    await waitFor(
      client,
      `document.querySelector('.auth-status')?.textContent.includes(${JSON.stringify(e2eVisitorName)})`,
      'registered visitor',
    )
    await waitFor(client, headerIncludesText('实名会话'), 'registered visitor session badge')
    await client.send('Emulation.setDeviceMetricsOverride', {
      deviceScaleFactor: 1,
    ...
    if (mock) {
      ...
      await waitFor(client, `document.querySelector('.page-heading h1')?.textContent.includes('游客购票工作台')`, 'booking page from empty orders')
    }
    if (mock) {
      await waitFor(
        client,
    ...
    if (mock) {
      ...
      await waitFor(client, includesText('票品接口暂不可用，当前展示演示票种'), 'catalog fallback alert')
      ...
      catalogFallbackState = await evaluate(
        ...
        `(() => ({
          ...
          hasCatalogAlert: document.body.textContent.includes('票品接口暂不可用，当前展示演示票种'),
          hasDemoHint: document.body.textContent.includes('当前仅可浏览演示数据，不能创建订单'),
      ...
      await clickDateChip(client, '明日 06-30')
      await waitFor(client, includesText('时段接口暂不可用，当前展示演示时段'), 'time slot fallback alert')
      ...
      timeSlotFallbackState = await evaluate(
        ...
        `(() => ({
          ...
          hasDemoHint: document.body.textContent.includes('当前仅可浏览演示数据，不能创建订单'),
          hasDemoSlot: document.body.textContent.includes('08:30-10:30'),
          hasRealProduct: document.body.textContent.includes('Adult Ticket Long Display Name'),
          hasTimeSlotAlert: document.body.textContent.includes('时段接口暂不可用，当前展示演示时段'),
        }))()`,
      )
    ...
    if (mock) {
      ...
      await clickDateChip(client, '演示日 07-01')
    ...
    await waitFor(
      client,
      `document.querySelector('.mobile-order-detail-drawer')?.textContent.includes('库存以支付结果为准')`,
      ...
      notPayablePaymentState = await evaluate(
        ...
        `(() => {
          ...
          return {
            ...
            hasRetryPayLabel: Boolean(drawer?.textContent.includes('继续模拟支付')),
      ...
      cancelNotAllowedState = await evaluate(
        ...
        `(() => {
          ...
          return {
            ...
            hasPayAction: visibleEnabledButtonTexts.some((text) => text.includes('继续模拟支付')),
      ...
      orderDetailErrorState = await evaluate(
        ...
        `(() => {
          ...
          return {
            ...
            hasPayAction: Boolean(drawer?.textContent.includes('继续模拟支付')),
    ...
    if (mock) {
      ...
      await waitFor(client, headerIncludesText('游客未登录'), 'logged-out session badge after logout')
    ...
    await clickSideMenuItem(client, '后台管理')
    await waitFor(client, `document.querySelector('.page-heading h1')?.textContent.includes('后台运营工作台')`, 'admin workspace shell')
    await waitFor(client, includesText('mock 运营预览'), 'admin mock boundary copy')
    await waitFor(client, includesText('请先登录管理员账号'), 'admin orders login gate')
    const adminLoggedOutState = await evaluate(
      client,
      `(() => ({
        hasAccessCard: Boolean(document.querySelector('.admin-access-card')),
        hasLockedOperationCards: document.querySelectorAll('.admin-operation-card').length >= 3,
    ...
    await fillPlaceholder(client, 'admin', 'admin')
    await fillPlaceholder(client, 'demo password', 'demo-secret')
    await clickSelector(client, '.admin-login-card button[type="submit"]')
    ...
    adminShellState = await evaluate(
      ...
      `(() => {
        ...
        return {
          ...
          hasCsrfBoundary: document.body.textContent.includes('CSRF Header'),
          hasHttpOnlyBoundary: document.body.textContent.includes('HTTP-only Cookie'),
          hasMockAuthMode: document.body.textContent.includes('Mock Auth'),
          hasMockOrdersMode: document.body.textContent.includes('Mock Orders'),
          hasMockReportsMode: document.body.textContent.includes('Mock Reports'),
          hasMockAuditMode: document.body.textContent.includes('Mock Audit'),
          hasMockCheckInAuditMode: document.body.textContent.includes('Mock Check-in Audit'),
          hasMockCheckInFailureAuditMode: document.body.textContent.includes('Mock Failure Audit'),
          hasMockPreview: document.body.textContent.includes('mock 运营预览'),
          ...
          hasOperationsBoundaryCopy:
            document.body.textContent.includes('后台操作按 read-model、状态变更、审计导出分区推进') &&
          ...
          hasBatchCheckInPanel: document.body.textContent.includes('批量票码核验') &&
            document.body.textContent.includes('状态变更 POST，只提交 ticketCodes'),
          ...
          hasSecondaryGrid: Boolean(document.querySelector('.admin-secondary-grid')),
          hasWorkbenchGrid: Boolean(document.querySelector('.admin-workbench-grid')),
          ...
          hasCheckInAuditExportPanel: document.body.textContent.includes('核验审计导出') &&
            document.body.textContent.includes('只读 GET：按票码、订单号、操作人和日期导出核验审计 CSV/XLSX'),
          hasCheckInFailureAuditPanel: document.body.textContent.includes('核验失败审计') &&
            document.body.textContent.includes('只读 GET：按失败码、票码、操作人和日期定位核验业务失败尝试'),
          ...
          hasExportJobsBoundary: document.body.textContent.includes('异步导出任务') &&
            document.body.textContent.includes('创建任务必须走管理员会话和 CSRF') &&
            document.body.textContent.includes('不提交 adminUserId、任务状态、fileName、storageKey 或下载链接') &&
            document.body.textContent.includes('前端不读取 storageKey'),
          hasMockExportJobsMode: document.body.textContent.includes('Mock Export Jobs'),
    ...
    adminExportJobCreateState = await evaluate(
      ...
      `(() => ({
        ...
        keepsCreateBoundary: document.body.textContent.includes('前端只提交 exportType、fileFormat 和白名单 filters'),
    ...
    adminReportState = await evaluate(
      ...
      `(() => {
        ...
        return {
          ...
          hasReadOnlyBoundary: document.body.textContent.includes('报表接口为只读 GET'),
    ...
    adminBatchCheckInState = await evaluate(
      ...
      `(() => {
        ...
        return {
          ...
          hasBoundary: text.includes('状态变更 POST，只提交 ticketCodes') &&
    ...
    adminBatchUndoCheckInState = await evaluate(
      ...
      `(() => {
        ...
        return {
          ...
          hasBoundary: text.includes('状态变更 POST，只提交 ticketCodes 和可选 reason') &&
            text.includes('逐票业务失败不阻断同批其他票码'),
          hasResultSummary: text.includes('共 2 个票码，成功 1 个，失败 1 个。'),
    ...
    adminCheckInFailureLogSearchState = await evaluate(
      ...
      `(() => {
        ...
        return {
          ...
          hasReadOnlyBoundary: text.includes('只读 GET'),
    ...
    adminRefundLogSearchState = await evaluate(
      ...
      `(() => {
        ...
        return {
          ...
          hasReadOnlyBoundary: text.includes('跨订单只读 GET'),
    ...
    adminPartialRefundState = await evaluate(
      ...
      `(() => {
        ...
        return {
          ...
          hasBoundary: drawerText.includes('只提交 itemNos 和 reason') &&
    ...
    adminFullRefundState = await evaluate(
      ...
      `(() => {
        ...
        return {
          ...
          hasBoundary: drawerText.includes('只提交 reason') &&
    ...
    adminRefundAuditState = await evaluate(
      ...
      `(() => {
        ...
        return {
          ...
          hasReadOnlyBoundary: drawerText.includes('只读 GET，不提交操作人、金额或状态。'),
          hasNoInternalAdminId: !drawerText.includes('adminUserId'),
    ...
    await waitFor(client, includesText('订单只读详情'), 'admin order detail drawer')
    ...
    adminDetailState = await evaluate(
      ...
      `(() => ({
        ...
        hasRefundBoundary: document.body.textContent.includes('只提交 reason') &&
    ...
    adminDetailState = {
      ...
      ...(await evaluate(
        ...
        `(() => ({
          ...
          hasUsedTicketState: document.body.textContent.includes('USED'),
          disablesCheckedInAction: [...document.querySelectorAll('.admin-check-in-action')].some((button) =>
            button.textContent.includes('核验') && button.disabled
          ),
    ...
    await waitFor(
      client,
      `(() => {
        const summary = document.querySelector('.admin-access-summary')
        return window.matchMedia('(max-width: 960px)').matches &&
          summary &&
          getComputedStyle(summary).gridTemplateColumns.split(' ').length === 1
      })()`,
      'admin intermediate authenticated access summary layout',
    )
    adminIntermediateState = await evaluate(
      ...
      `(() => {
        ...
        return {
          clientWidth,
          hasAccessSummary: Boolean(summary),
          hasAuthenticatedWorkbench: Boolean(document.querySelector('.admin-workbench-grid.is-authenticated')),
          scrollWidth,
    ...
    adminTabletState = await evaluate(
      ...
      `(() => {
        ...
        return {
          clientWidth,
          hasAccessSummary: Boolean(document.querySelector('.admin-access-summary')),
          hasAuthenticatedWorkbench: Boolean(document.querySelector('.admin-workbench-grid.is-authenticated')),
          hasExportJobsPanel: Boolean(document.querySelector('.admin-export-jobs-panel')),
