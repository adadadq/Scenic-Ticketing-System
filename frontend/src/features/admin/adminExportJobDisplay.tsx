...
const filterNameMap: Partial<Record<keyof AdminExportJobFilters, string>> = {
  dateFrom: '起始日期',
  dateTo: '截至日期',
  failureCode: '失败原因',
  includeEmpty: '补齐空档',
  operatorUsername: '操作人账号',
  orderNo: '订单号',
  refundType: '退款类型',
  ticketCode: '票码',
}
...
export function filterTagLabel(key: string, value: string | boolean) {
  const label = filterNameMap[key as keyof AdminExportJobFilters] ?? key
  const displayValue = value === true ? '是' : value
