...
export function AdminRefundAuditSearchErrorDetails({ error }: { error: unknown }) {
  const apiError = error instanceof ApiError ? error : null

  return (
    <Flex className="api-error-details" gap={8} vertical>
      <Text>退款审计日志暂时无法检索，请稍后重试。</Text>
      <Text type="secondary">请保留错误码和请求编号，便于后端定位管理员会话、筛选参数或分页读取问题。</Text>
      {apiError ? (
