    ...
    <Space className="admin-card-stack admin-export-jobs-panel" orientation="vertical" size={14}>
      <div className="admin-section-heading admin-export-jobs-heading">
        <div>
          ...
          <Text type="secondary">大文件先创建任务，再查看生成状态；创建任务必须走管理员会话和防伪校验。</Text>
        </div>
        <Space size={8} wrap>
          <Tag color={adminExportJobsMode === 'api' ? 'blue' : 'gold'}>
            {adminExportJobsMode === 'api' ? '真实导出任务' : '演示导出任务'}
          </Tag>
      ...
      <Alert
        ...
        type="info"
        title="任务边界"
        description="前端只提交导出类型、文件格式和允许的筛选条件；不提交管理员内部编号、任务状态、文件名、存储位置或下载链接。"
      />
      ...
      <Text className="admin-orders-footnote" type="secondary">
        列表只展示公开任务编号、任务状态、错误码、申请人账号和文件名；下载必须等任务成功且文件元数据存在，前端不读取后端存储位置。
      </Text>
    </Space>
  )
}
