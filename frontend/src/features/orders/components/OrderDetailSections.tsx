...
function getOrderDetailStateCard({
  ...
  if (isPaymentBlocked) {
    return {
      detail: '支付接口返回订单状态变化，请先刷新订单详情再继续操作。',
  ...
  if (canPay || canCancel) {
    return {
      detail: '可继续模拟支付，或在支付前取消订单；库存以最终支付结果为准。',
  ...
  if (order.paymentStatus === 'PAID') {
    return {
      detail: '订单已支付，等待票码返回或刷新订单详情；不要在前端推断票码。',
      icon: <CheckCircleFilled />,
      label: '已支付',
      tag: '只读',
      tone: 'success',
  ...
  return {
    detail: '当前详情只展示后端返回的订单状态和票码，不在前端改写订单结果。',
    icon: <InfoCircleOutlined />,
    label: '详情只读',
    tag: '只读',
    tone: 'default',
  ...
  return (
    <Flex className="order-detail-actions" gap={10} wrap>
      <Button
        ...
        type="primary"
      >
        {isPaymentBlocked ? '不可支付' : canPay ? '继续模拟支付' : '无需支付'}
      </Button>
      <Popconfirm
        cancelText="先不取消"
        disabled={!canCancel}
