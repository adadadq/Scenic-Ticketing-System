import { adminReportExportsApi } from '../../shared/api/endpoints'
...
export function buildAdminProductBreakdownCsvText(rows: AdminProductBreakdown[]) {
  const body = rows.map((row) => adminProductBreakdownCsvHeaders.map((header) => escapeCsvCell(row[header])).join(','))
...
export function buildAdminProductBreakdownXlsxBlob(rows: AdminProductBreakdown[]) {
  return buildWorkbookBlob('Product Breakdown', [
    ...
    ...rows.map((row) => adminProductBreakdownCsvHeaders.map((header) => row[header])),
  ])
}
...
export async function downloadAdminProductBreakdownCsv(params: AdminReportParams = {}) {
  const normalizedParams = normalizeAdminReportParams(params)
  const blob = adminReportsMode === 'api'
    ? await adminReportExportsApi.productBreakdownCsv(normalizedParams)
    : new Blob([buildAdminProductBreakdownCsvText(listMockAdminProductBreakdown(normalizedParams))], {
      type: 'text/csv;charset=utf-8',
    })
...
export async function downloadAdminProductBreakdownXlsx(params: AdminReportParams = {}) {
  const normalizedParams = normalizeAdminReportParams(params)
  const blob = adminReportsMode === 'api'
    ? await adminReportExportsApi.productBreakdownXlsx(normalizedParams)
    : buildAdminProductBreakdownXlsxBlob(listMockAdminProductBreakdown(normalizedParams))
